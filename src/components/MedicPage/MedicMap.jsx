...
import 'mapbox-gl/dist/mapbox-gl.css';
import MapboxDirections from '@mapbox/mapbox-gl-directions/dist/mapbox-gl-directions';
import io from 'socket.io-client'; // التأكد من استيراد Socket.io هنا فقط
import './Loader.css'; // استيراد ملف CSS للودر الرادار
...
const MedicMap = () => {
  ...
  const [estimatedTime, setEstimatedTime] = useState(''); // لتخزين المدة الزمنية المقدرة
  const [isLoading, setIsLoading] = useState(true); // حالة البحث (جاري البحث)
  const socketRef = useRef(null); // تخزين اتصال Socket.io هنا
  ...
  // الاتصال بـ Socket.IO فقط في هذا المكون
  useEffect(() => {
    socketRef.current = io('http://localhost:3000'); // الاتصال بالخادم
    const fetchCase = () => {
      socketRef.current.on('locationUpdate', (updatedLocation) => {
        if (updatedLocation) {
          setCaseLocation([updatedLocation.longitude, updatedLocation.latitude]); // تحديث موقع الحالة من الخادم
          setIsLoading(false); // إنهاء حالة البحث
        } else {
    ...
    return () => {
      socketRef.current.off('locationUpdate'); // إزالة الاستماع عند تدمير المكون
      socketRef.current.disconnect(); // قطع الاتصال بـ Socket.IO عند تدمير المكون
      clearInterval(intervalId); // مسح التكرار
    };
  }, []);
